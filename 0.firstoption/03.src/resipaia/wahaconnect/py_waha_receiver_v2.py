...
from resipaia.interpretartexto.py_main_processor_v2 import process_message_logic # Importa a lógica principal da v2
...
class WahaReceiver:
    ...
    async def process_message_from_waha(self, message_details: dict):
        ...
        session_id = waha_payload.get("session", "default")
        ...
        if chat_id:
            ...
            logging.info(f"processor_response: {chat_id}: {processor_response}")
            
            # Extrai a mensagem de resposta do processador
            response_message = processor_response.get("message", "Erro ao processar a mensagem.")
            if isinstance(response_message, dict) and "parts" in response_message and len(response_message["parts"]) > 0:
                response_message_text = response_message["parts"][0].get("text", "")
            else:
                response_message_text = str(response_message)
            ...
            logging.info(f"Enviando resposta final para {chat_id}, response_payload: {response_payload}.")
