...
def handle_pix_initiation(message_details: dict) -> dict:
    phone_number = message_details.get('from')
    logging.info(f"Simulando geração de Pix. Integração desativada para {phone_number}.")
    return {
        "to": phone_number,
...
def handle_pix_status_check(message_details: dict) -> dict:
    phone_number = message_details.get('from')
    logging.info(f"Simulando verificação de status do Pix. Integração desativada para {phone_number}.")
    return {
        "to": phone_number,
        "message": "A verificação do status do Pix está temporariamente desativada. O status atual é 'pendente'."
    }
...
def main():
    if len(sys.argv) > 1:
        ...
        try:
            ...
            response = process_message_logic(message_details) # Chamada para a nova função
