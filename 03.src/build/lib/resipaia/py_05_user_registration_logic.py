...
def check_user(supabase_client: Client, message_details: dict) -> dict:
    ...
    try:
        response = supabase_client.table('cadastro_pessoas_fisica').select('id', 'phone_number').eq('phone_number', phone_number).single().execute()
        if response.data:
            return {"status": "user_found", "message": f"Bem-vindo de volta, {phone_number}!"}
        else:
