...
## 1.1. Visão Geral
Este documento detalha a proposta para a criação de um sistema de reserva de quiosques e quadras de beach tennis via WhatsApp, utilizando Waha, n8n, Docker, PostgreSQL e Gemini. O sistema permitirá que usuários cadastrados realizem reservas, efetuem pagamentos via Pix e gerenciem cancelamentos com notificação de lista de espera.
...
### 1.2.3. Banco de Dados (PostgreSQL)
- Um contêiner Docker para o PostgreSQL, que armazenará todas as informações cruciais: cadastro de pessoas físicas, detalhes dos quiosques e quadras, registros de reservas, status de pagamento e a lista de espera.
...
1.  **1.4.1. Definição do Esquema do Banco de Dados:** Criar as tabelas necessárias no PostgreSQL.
2.  **1.4.2. Configuração Docker Compose:** Montar o ambiente com Waha, n8n e PostgreSQL.
3.  **1.4.3. Desenvolvimento dos Workflows n8n:** Implementar a lógica de negócios, integração com WhatsApp, PostgreSQL e Gemini.
